...
const commentHandler = (settings, commentParserToESTreeOptions) => {
  ...
  return (commentSelector, jsdoc) => {
    ...
    const ast = commentParserToESTree(jsdoc, mode, commentParserToESTreeOptions);
...
function estreeToString(node, opts = {}) {
  ...
  if (node.type.startsWith('JsdocType')) {
    ...
    node, opts.jtppStringificationRules)}}`;
  }
...
function JsdocTag(node, opts) {
  ...
  if (name && postTag === '' && postType === '') {
    ...
    if (descriptionLines.length > 0) {
      ...
      /* v8 ignore next 3 */
      if (firstDescLine.description === '' && firstDescLine.delimiter === '' && firstDescLine.postDelimiter === '' && firstDescLine.initial === '') {
        // Empty first line, check if there's a second line with formatting
        if (descriptionLines.length > 1) {
          const nameLine = descriptionLines[1];
          result += `${nameLine.initial}${nameLine.delimiter}${nameLine.postDelimiter}${name}${postName}${nameLine.description}`;
          nameLineOffset = 2;
        } else {
          /* v8 ignore start */
